...
from utils import save_images, save_source, save_image_1
...
flags.DEFINE_string("checkpoint_dir", './checkpoints/checkpoints/',
                    "Directory name to save the checkpoints")

flags.DEFINE_string("save_dir",'./age/test_output_new/',
                    "Directory name to save the sample images")

flags.DEFINE_string("test_data_dir", './test_images/', "test images")
...
# RAHUL CHANGES: Changed all print statements to be compatible with python 3. changed the image read and write to the cv2's functions instead of scipy's functions
def my_train():
...
def generate_images_from_folder(model, sess, test_data_dir=None, train_data_dir=None):
    ...
    for i in range(len(paths)):
        ...
        for j in range(1, generator.n_classes):
            ...
            image = np.reshape(samples[0, :, :, :], (128, 128, 3))
            # generator.save_batch(samples, paths, FLAGS.save_dir, index=j, if_target=True)
            print(save_image_1(image, [1, 1], os.path.join(FLAGS.save_dir, paths[i] + '_' + str(j) + '.jpg')))
